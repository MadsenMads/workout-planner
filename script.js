...
// ... [Rest of your existing functions, such as exportToHTML and downloadHTMLFile] ...
...
// Function to generate table from URL parameters
function generateTableFromUrl() {
    const urlParams = new URLSearchParams(window.location.search);
    ...
    if (equipment && exercise && date) {
        document.getElementById("equipment-select").value = decodeURIComponent(equipment);
        updateExerciseButtons(decodeURIComponent(equipment));

        // Assuming the first row is generated by default
        const workoutTable = document.getElementById("workout-table");
        const row = workoutTable.insertRow(-1);
        const cellExercise = row.insertCell(0);
        cellExercise.textContent = decodeURIComponent(exercise);

        // Add empty cells for sets
        for (let i = 1; i <= 5; i++) {
